...
function formatDate(d2, locale = "en-GB") {
...
function generateAtomFeed(cfg, idx, options, limit) {
  const base = cfg.baseUrl ?? "";
  ...
  const createEntry = (slug2, content) => {
    const published = content.date;
    const updated = content.modifiedDate ?? content.date;
    return `<entry>
    <title>${escapeHTML(content.title)}</title>
    <link href="https://${joinSegments(base, encodeURI(slug2))}" />
    <link rel="alternate" type="text/markdown" href="https://${joinSegments(base, encodeURI(slug2))}.html.md" />
  ...
  const entries = Array.from(idx).sort(([_2, f1], [__, f2]) => {
    if (f1.date && f2.date) {
  ...
  }).map(([slug2, content]) => createEntry(simplifySlug(slug2), content)).slice(0, limit ?? idx.size).join("\n");
  const subtitle = `${limit ? lastFewNotesText(limit) : recentNotesText} on ${escapeHTML(pageTitle)}`;
  const feedUpdated = idx.get("index")?.date?.toISOString() ?? (/* @__PURE__ */ new Date()).toISOString();

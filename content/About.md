---
dg-publish: true
---
hey! you can call me tom, or pl4nty (/ˈplaːnti/). I'm a 20-something guy from Australia, living in Melbourne after spending time all over the country. I enjoy breaking and making all sorts of systems, especially software. I'm also fond of [[Books|novels]], nature, and opera (as well as [art song](https://en.wikipedia.org/wiki/Art_song)).

if we happen to cross paths, I’m always down to grab coffee or say hi! I travel to Sydney pretty often, and some other places too:
* [PECAN+ 2026](https://pecanplus.org/) in Perth
* [[un]prompted](https://unprompted.au/) in Sydney
* [BSides Canberra](https://www.bsidesau.com.au/). I always try to attend, it was my first con!
* Possibly [ECSC 2026](https://ecsc2026.de/) in Germany, or [CyberCon 2026](https://www.cyberconference.com.au/) in Melbourne otherwise
* [BSides Perth 2026](https://bsidesperth.com.au/)
* Japan in December

some communities I'm involved with
* organiser of [PECAN+](https://pecanplus.org/)
* [Emu Exploit](https://emu.team/) capture-the-flag team, [ICC](https://icc.ecsc.eu/)/[ECSC](https://en.wikipedia.org/wiki/European_Cybersecurity_Challenge), and the aussie CTF scene in general
* the [Australian National University](https://www.anu.edu.au/), and its venerable [CompSci Students' Association](https://cs.club.anu.edu.au/)
* [National Youth Science Forum](https://www.nysf.edu.au/)
* [Workplace Ninjas Australia](https://au.wpninjas.global/)

open source
* my [most popular project](https://github.com/pl4nty/cloudflare-kubernetes-gateway) was an excuse to learn golang on a weekend 💀
* [Talos Linux distro for k8s](http://github.com/siderolabs/talos), particularly my RISC-V port. fully bootstrapped from 181 bytes of machine code. yes, it was painful
* Linux kernel and [U-Boot](https://en.wikipedia.org/wiki/Das_U-Boot), for those RISC-V boards
* k8s, OpenTelemetry, BuildKit, other misc CNCF projects
* anytime I find a simple bug. talk is cheap, send (good) patches

making
* keyboards, since 2019. mostly splits with gimmicks
* simple PCBs, like badgelife

micro🅱️oft
* [[Notes/Microsoft/Intune/index|Microsoft Intune]] and [[Notes/Microsoft/Windows/index|Windows]] from my work at [Devicie](https://devicie.com/)
* Windows Package Manager (winget) contributor - CLI, packaging tooling, [weird hacks](https://github.com/pl4nty/winget-extras), etc